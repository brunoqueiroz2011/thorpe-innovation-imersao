...
class UsersRouter extends ModelRouter<User> {
  ...
  envelope(document){
    ...
    resource._links.menu = `${this.basePath}/${resource._id}/todoList`
    return resource
  }
  ...
  findMenu = (req, resp, next) => {
    User.findById(req.params.id, "+todoList")
       .then(rest =>{
         if(!rest){
           throw new NotFoundError('User not found')
         }else{
           resp.json(rest.todoList)
           return next()
         }
       }).catch(next)
  }
  ...
  replaceMenu = (req, resp, next)=>{
    User.findById(req.params.id).then(rest=>{
        if(!rest){
          throw new NotFoundError('User not found')
        }else{
          rest.todoList = req.body //ARRAY de MenuItem
          return rest.save()
        }
      }).then(rest=>{
        resp.json(rest.todoList)
